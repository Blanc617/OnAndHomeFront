    ...
    <div className="admin-qna-list">
      ...
      {totalPages > 1 && (
        ...
        <div className="qna-stats">
          ...
          <p>
            미답변: {filteredQnaList.filter(q => !q.replies || q.replies.length === 0).length}개 / 
            답변완료: {filteredQnaList.filter(q => q.replies && q.replies.length > 0).length}개
          </p>
        </div>
      </div>
    </div>
